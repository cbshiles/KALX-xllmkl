...
namespace xll {
	...
	class SSTask : public vsl::SSTask<> {
		...
		std::map<MKL_INT,FPX> map; // Map Edit requests to results
	public:
...
//			mkl_free_buffers(); // MKL likes to hang on to memory
		...
		MKL_INT Compute(MKL_INT param, MKL_INT method)
		{
			...
			if (method) // only call TaskEdit
				status = vsl::SSTask<>::Compute(params, method);

			return status;
		}

		// use same param as in Compute call
		const xfp* Result(MKL_INT param)
		{
	...
	private:
		...
		static MKL_INT64 mask(MKL_INT param) {
			static struct params_ { MKL_INT64 mask; } params[] = {
				...
				{VSL_SS_MEAN|VSL_SS_2R_MOM|VSL_SS_VARIATION}, //#define VSL_SS_ED_VARIATION                            18
				{VSL_SS_MEAN|VSL_SS_COV}, //#define VSL_SS_ED_COV                                  19
